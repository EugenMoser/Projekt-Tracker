...
export function collectPushPayload(db: AnyDb, userId: string, since: Date | null): PushPayload {
  const bDb = db as BetterSQLite3Database<typeof schema>

  function withSince<T extends { updatedAt: Date }>(rows: T[]): T[] {
    return since ? rows.filter((r) => r.updatedAt.getTime() > since.getTime()) : rows
  }

  const orderTypesAll = bDb
    .select()
    .from(schema.orderTypes)
    .where(eq(schema.orderTypes.userId, userId))
    .all()
  const customersAll = bDb
    .select()
    .from(schema.customers)
    .where(eq(schema.customers.userId, userId))
    .all()
  const projectsAll = bDb
    .select()
    .from(schema.projects)
    .where(eq(schema.projects.userId, userId))
    .all()
  const tasksAll = bDb.select().from(schema.tasks).where(eq(schema.tasks.userId, userId)).all()
  const tagsAll = bDb.select().from(schema.tags).where(eq(schema.tags.userId, userId)).all()
  const timeEntriesAll = bDb
    .select()
  ...
  const taskTagsAll = bDb
    .select()
    .from(schema.taskTags)
    .where(eq(schema.taskTags.userId, userId))
    .all()
  const projectTasksAll = bDb
    .select()
    .from(schema.projectTasks)
    .where(eq(schema.projectTasks.userId, userId))
    .all()
  const timersAll = bDb.select().from(schema.timers).where(eq(schema.timers.userId, userId)).all()
  const appSettingsRow =
    bDb.select().from(schema.appSettings).where(eq(schema.appSettings.userId, userId)).get() ?? null
