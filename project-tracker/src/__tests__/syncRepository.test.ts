...
import { applyPull, collectPushPayload } from '../sync/syncRepository'
...
describe('collectPushPayload', () => {
  ...
  it('soft-deleted entity (deletedAt set) is included with deletedAt as ISO', () => {
    ...
    db.update(schema.orderTypes).set({ deletedAt: T1, updatedAt: T1 }).run()
    const payload = collectPushPayload(db, U, null)
...
describe('applyPull', () => {
  ...
  it('updates existing entity when server updatedAt is newer (LWW)', () => {
    const db = makeDb()
    db.insert(schema.users)
      .values({ id: U, displayName: 'Owner', tier: 'pro', createdAt: T0, updatedAt: T0 })
      .run()
    db.insert(schema.orderTypes)
      .values({
        id: OT,
  ...
  it('full-replaces timers (delete existing, insert server timers)', () => {
    ...
    db.insert(schema.users)
      ...
      .run()
    db.insert(schema.orderTypes)
      .values({ id: OT, userId: U, name: 'H', digit: 1, createdAt: T0, updatedAt: T0 })
      .run()
    db.insert(schema.customers)
      .values({
        id: CU,
  ...
  it('full-replaces taskTags', () => {
    ...
    db.insert(schema.users)
      ...
      .run()
    db.insert(schema.tasks)
      .values({ id: TA, userId: U, description: 'A', createdAt: T0, updatedAt: T0 })
      .run()
    const TAG = '00000000-0000-0000-0000-000000000007'
    db.insert(schema.tags)
      .values({ id: TAG, userId: U, title: 'wichtig', createdAt: T0, updatedAt: T0 })
      .run()
...
describe('sortOrder round-trip', () => {
  it('carries sortOrder into the push payload', () => {
    const db = makeDb()
    seedBase(db)
    db.update(schema.projects).set({ sortOrder: 2000 }).where(eq(schema.projects.id, PR)).run()
