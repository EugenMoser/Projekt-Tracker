...
function nextSortOrder(userId: string): number {
  const row = db
    .select({ value: max(schema.projects.sortOrder) })
    .from(schema.projects)
...
export function getProjectTotalSeconds(userId: string, projectId: string): number {
  const result = db
    .select({ total: sum(schema.timeEntries.durationSeconds) })
...
export function moveProject(
  ...
  db.transaction(() => {
    ...
    if (key === null) {
      throw new Error(`Cannot place project ${projectId} between ${prevId} and ${nextId}`)
    }
...
function rebalance(userId: string): void {
  ...
  rows.forEach((row, index) => {
    db.update(schema.projects)
      ...
      .where(and(eq(schema.projects.id, row.id), eq(schema.projects.userId, userId)))
      .run()
  })
}
