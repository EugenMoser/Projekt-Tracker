...
export function getActiveTimer(userId: string) {
  return db.select().from(schema.timers).where(eq(schema.timers.userId, userId)).get() ?? null
}
...
export function stopTimer(userId: string, taskId: string, notes?: string): void {
  ...
  db.delete(schema.timers)
    .where(and(eq(schema.timers.id, timer.id), eq(schema.timers.userId, userId)))
    .run()
}
